...
The project will operate transparently, collaboratively, ethically, and in alignment with the interests of security-as-code technology end-users. Project proposals, timelines, and status must not merely be open, but also easily visible to and discoverable by outsiders.

## Evolution of the project Governance

Most large, complex open source communities have both a business and a technical governance model. The project has a project-level Governance Committee, a project-level Technical Committee.

This Technical Committee Charter reflects the scope and expectations of the Technical Committee ("TC") with the maintainers, the consumers, and the ecosystem of the project. The charter will not be perfect, and so has a simple amendment process – a TC member proposes changes to be discussed and voted on by the full TC.
...
- Ensuring new maintainer(s) for a particular repo are identified when a previous maintainer leaves.
- Resolving a dispute over whether or not a particular change is considered "breaking".
- Ensuring repositories and releases conform to the [established requirements](https://github.com/security-as-code/.github/blob/main/CONTRIBUTING.md#repository-requirements).
- Reviewing [MAIN REPO](link) PRs and [EPs](link).
- Creating and reviewing [projects](https://github.com/orgs/security-as-code/projects).
